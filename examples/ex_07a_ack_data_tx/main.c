...
void DEBUG_USART_IRQHandler(void)
{
    ...
    if(USART_GetITStatus(DEBUG_USART, USART_IT_RXNE) != RESET)
    {
    	...
    	ringBuffer_writeByte(&usart4_receive_buffer, temp);

//    	ringBuffer_print(&usart4_receive_buffer);
    	//DEBUG_USART_SendByte(tx_msg[1+counter]);
//    	DW1000_SendByte(tx_msg, 1);
//     	counter++;
//     	if(counter >= 16)
//     	{
// //    		DEBUG_USART_SendArray(tx_msg, sizeof(tx_msg));
// //    		DW1000_SendByte(tx_msg, sizeof(tx_msg));
//     		dw1000_send_flag = 1;
//     		counter = 0;
//     	}
    }
...
uint8_t DW1000_sendByte(uint8_t buff[], int length)
{
    /* Write frame data to DW1000 and prepare transmission. See NOTE 7 below.*/
    dwt_writetxdata(length, buff, 0); /* Zero offset in TX buffer. */
    dwt_writetxfctrl(length, 0, 0); /* Zero offset in TX buffer, no ranging. */
...
int main(void)
{
    ...
    while (1)
    {
//    	if(!DW1000_sendByte(tx_msg, sizeof(tx_msg)))
//    		if(!DW1000_sendByte(tx_msg, sizeof(tx_msg)))
//    			;
    	if(usart4_receive_buffer.headPosition != usart4_receive_buffer.tailPosition)
    	{
    		counter = 0;
            tx_buff[counter++] = 0x61;
            tx_buff[counter++] = 0x88;
            tx_buff[counter++] = 0 + tx_frame_nb;
           	tx_buff[counter++] = 0xCA;
           	tx_buff[counter++] = 0xDE;
            tx_buff[counter++] = 'X';
            tx_buff[counter++] = 'R';
           	tx_buff[counter++] = 'X';
           	tx_buff[counter++] = 'T';
            while(ringBuffer_readByte(&usart4_receive_buffer, &pdata))
            {
                tx_buff[counter++] = pdata;
                if(counter > 120)
                    break;
            }
            tx_buff[counter++] = 0x00;
            tx_buff[counter++] = 0x00;

            DW1000_sendByte(tx_buff, counter);
            for(i = 0; i < 30000; i++)
            {;}
//                DEBUG_USART_SendArray(tx_buff, counter);


//            if(DW1000_sendByte(tx_buff, counter))
//            {
//                DEBUG_USART_SendArray(tx_buff, counter);
//            }
